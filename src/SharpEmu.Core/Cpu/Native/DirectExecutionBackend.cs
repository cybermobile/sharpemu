...
#pragma warning restore CS0649
	...
	private bool ActiveForcedGuestExit
	{
		...
		set
		{
			...
			{
				Volatile.Write(ref _forcedGuestExit, value);
			}
		}
	}
	...
	public bool TryExecute(CpuContext context, ulong entryPoint, Generation generation, IReadOnlyDictionary<ulong, string> importStubs, IReadOnlyDictionary<string, ulong> runtimeSymbols, CpuExecutionOptions executionOptions, out OrbisGen2Result result)
	{
		...
		Volatile.Write(ref _forcedGuestExit, false);
		...
		out string? error)
	{
		returnValue = 0;
		error = null;
		if (Volatile.Read(ref _forcedGuestExit))
		{
	...
	private void RunGuestThread(GuestThreadState thread, string reason)
	{
		...
		try
		{
			...
			if (TryResolveBlockedResumeRax(blockWaiter, resumeHandler: null, out var resumedRax))
			{
		...
		try
		{
			...
			*(ulong*)(ptr2 + offset) = hostRspSlot;
			...
			try
			{
				...
				if (name == "tbb_thead")
				{
					nativeReturn = unchecked((ulong)(uint)RunGuestEntryStub(
						ptr,
						hostRspSlot,
						requireNativeWorker: true));
				}
				else
				{
					...
					nativeReturn = CallNativeEntryU64((nint)ptr);
				}
